...
group = driver.create_auto_scale_group(name='libcloud-group',
                       min_size=1, max_size=5, cooldown=300,
                       termination_policies=AutoScaleTerminationPolicy.\
                                          CLOSEST_TO_NEXT_CHARGE,
                       size=size, location=location, 
                       # region matching Dallas datacenter
                       ex_region='na-usa-central-1')
pprint(group)

policy = driver.create_auto_scale_policy(group=group,
...
alarm = driver.create_auto_scale_alarm(name=\
                       'libcloud-alarm',
                       policy=policy,
